...
class OrcaServingTest(unittest.TestCase):
    ...
    def test_dynamic_iteration_batching_and_sequence_cleanup(self):
        ...
        snapshot = scheduler.resource_snapshot()
        self.assertGreater(snapshot["orca_scheduler_iterations_total"], 0)
        self.assertGreaterEqual(snapshot["orca_prefill_batch_size_max"], 1)
        self.assertGreaterEqual(snapshot["orca_decode_batch_size_max"], 2)
